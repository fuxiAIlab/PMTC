...
class DataCollatorForLanguageModeling:
    ...
    def mask_tokens(self, inputs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.seperate_design_id:
            cut_index = int(labels.shape[1] / 3)
            labels = labels[:, :cut_index]
        else:
            if self.use_time_embed:
                cut_index = int(labels.shape[1] / 2)
                labels = labels[:, :cut_index]
        ...
        if self.use_time_embed:
            inputs[:, :cut_index][indices_replaced] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)
        else:
        ...
        if self.use_time_embed:
            inputs[:, :cut_index][indices_random] = random_words[indices_random]
        else:
