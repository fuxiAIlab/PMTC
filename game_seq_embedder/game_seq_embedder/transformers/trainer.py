...
class Trainer:
        ...
        for epoch in range(epochs_trained, int(np.ceil(num_train_epochs))):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if seperate_design_id:
                    ...
                    assert inputs['input_ids'].shape[1] % 3 == 0
                    cut_index1 = int(inputs['input_ids'].shape[1] / 3)
                    cut_index2 = cut_index1 * 2
                    behave_ids = inputs['input_ids'][:, :cut_index1]
                    design_ids = inputs['input_ids'][:, cut_index1:cut_index2]
                    timestamps = inputs['input_ids'][:, cut_index2:]
                    inputs['time_gaps'] = timestamps
                    inputs['input_ids'] = behave_ids
                    inputs['design_ids'] = design_ids
                    assert inputs['labels'].shape == behave_ids.shape == design_ids.shape == timestamps.shape
                else:
                    if use_time_embed:
                        cut_index = int(inputs['input_ids'].shape[1] / 2)
                        origin_input_ids = inputs['input_ids'][:, :cut_index]
                        time_gaps = inputs['input_ids'][:, cut_index:]
                        inputs['time_gaps'] = time_gaps
                        inputs['input_ids'] = origin_input_ids
                        # TEST on Index 3
                        # mask_indices = [i for i, x in enumerate(inputs['labels'][3]) if x != -100]
                        # origin_input_ids[3][mask_indices]
                        assert inputs['labels'].shape == time_gaps.shape == origin_input_ids.shape
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        # input keys: dict_keys(['input_ids', 'labels'])
        # {'input_ids': tensor([[   5, 1032, 1493,  ...,    1,    1,    1],
        #         [   4, 1032, 1493,  ...,    1,    1,    1],
        #         [   5,  961, 4143,  ..., 2603, 1081,   28],
        #         ...,
        #         [   5, 6240, 1307,  ...,    1,    1,    1],
        #         [   5,  961,   19,  ..., 2603, 1081,   28],
        #         [   5,  961, 2572,  ...,    1,    1,    1]]),
        ...
        if self.args.fp16 and _use_native_amp:
            ...
            if model_name == 'TransfoXLLMHeadModel':
    ...
    def evaluate(self, eval_dataset: Optional[Dataset] = None, use_time_embed=0, logger=None,
                 seperate_design_id=False
                 ) -> Dict[str, float]:
        """
