...
from transformers.modeling_bert import BaseModelOutput
...
class BertEncoderMeanPool(nn.Module):
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            hidden_states = layer_outputs[0]  # batch_size x max_seq_len x feature_dim
            mean_pool_hidden = torch.mean(hidden_states, dim=1)

            if i == len(self.layer) - 1:
                for hidden_i, hidden_state in enumerate(hidden_states):
                    hidden_states[hidden_i] = hidden_state + mean_pool_hidden[hidden_i]
...
class BertModelMeanPool(BertPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.encoder = BertEncoderMeanPool(config)
...
class BertForMaskedLMMeanPool(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        ...
        self.bert = BertModelMeanPool(config)
