...
from .bpe_model import BpeTimeStampRnn
# from .bert_model_add_mean_pool import BertModelMeanPool
...
class BertEmbeddingsTimeEmbed(nn.Module):
    ...
    def __init__(self, config):
        ...
        # is_timestamp_rnn
        if hasattr(config, 'is_timestamp_rnn'):
            self.is_timestamp_rnn = config.is_timestamp_rnn
            self.timestamp_rnn_model = BpeTimeStampRnn()
        else:
            self.is_timestamp_rnn = False
            self.timestamp_rnn_model = None
        ...
        if self.use_sinusoidal:
            ...
            self.sinusoidal_time_embeddings = positionalencoding1d_batch(86400 + 1, self.hidden_size)
        else:
        ...
        # position_ids (1, len position emb) is contiguous in memory and exported when serializedisinstance(timestamps[0][0], int)
        self.register_buffer("position_ids", torch.arange(config.max_position_embeddings).expand((1, -1)))

    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, timestamps=None,
                design_ids=None):

        if timestamps is not None:
            if self.is_timestamp_rnn:
                assert isinstance(timestamps[0][0], tuple)
            else:
                assert isinstance(timestamps[0][0], (int, np.int64, np.int32))
        ...
        pad_seq_length = inputs_embeds.shape[1]

        if self.use_sinusoidal:
            if self.is_timestamp_rnn:
                # Input id shape: batch_size x 5112
                # timestamps shape: List[List[Tuple](长度为no padding的最大长度), List[Tuple]]
                time_gap_embeddings = torch.zeros_like(inputs_embeds)
                for timestamp_i, timestamp in enumerate(timestamps):
                    timestamp_lens = [len(x) for x in timestamp]
                    max_len = max(timestamp_lens)
                    timestamp_pad = torch.zeros((len(timestamp), max_len), dtype=torch.long)
                    for subword_i, subword_timestamp in enumerate(timestamp):
                        timestamp_pad[subword_i][:len(subword_timestamp)] = torch.tensor(subword_timestamp)
                    timestamp_embedding = self.sinusoidal_time_embeddings[timestamp_pad].to(device)
                    time_gap_embedding = self.timestamp_rnn_model(timestamp_embedding, timestamp_lens)
                    time_gap_embeddings[timestamp_i][:len(time_gap_embedding)] = time_gap_embedding
            else:
                timestamps_pad = torch.zeros((len(timestamps), pad_seq_length)).long()
                for t_i, timestamp in enumerate(timestamps):
                    timestamps_pad[t_i][:len(timestamp)] = torch.tensor(timestamp)
                time_gap_embeddings = self.sinusoidal_time_embeddings[timestamps_pad].to(device)
        else:
            # batch fancy indexing
            time_gap_embeddings = self.time_gap_embeddings(timestamps).to(device)  # time_gaps: batch_size x seq_length
...
class BertForMaskedLMTimeEmbed(BertPreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            # prediction_scores: batch_size x seq_len x vocab_size
